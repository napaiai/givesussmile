...
'<re>데이터베이스 모델링</re>',
'<re>ACID</re>',
'<re>Isolation Level</re>',
'<re>데이터 독립성</re>',
'<re>[데이터 독립성]- ANSI SPARC 3계층</re>',
'<re>병행제어 = 동시성제어</re>',
'<re>[동시성제어]- 트랜잭션 직렬화</re>',
'<re>[동시성제어]- Timestamp</re>',
'<re>[동시성제어]- Locking</re>',
'<re>[동시성제어]- 2PL</re>',
'<re>[동시성제어]- Phantom Conflict</re>',
'<re>[동시성제어]- MVCC</re>',
'<re>[동시성제어]- 낙관적 검증</re>',
'<re>회복기법</re>',
'<re>[회복기법]- Log 기반</re>',
...
'<re>[회복기법]- 미디어</re>',
'<re>[회복기법]- WAL정책</re>',
'<re>[회복기법][WAL]- ARIES</re>',
'<re>[일관성]- SAGA</re>',
'<re>[일관성][SAGA]- Choreography-Based Saga</re>',
'<re>[일관성][SAGA]- Orchestration-Based Saga</re>',
'<re>DB 이상현상</re>',
'<re>정규화</re>',
'<re>역정규화</re>',
'<re>[정규화]-암스트롱 공리, 함수폐포, 자기참조관계</re>',
'<re>관계대수</re>',
'<re>관계해석</re>',
'<re>이벤트 소싱(Event Sourcing)</re>',
'<re>다차원 모델링</re>',
'<re>[다차원 모델링]- 스타 스키마</re>',
'<re>[SNA]- 중심성(Centrality)</re>',
'<re>CKAN(Comprehensive Knowledge Archive Network)</re>',
'<re>[DB Stakeholder]- DA</re>',
'<re>[DB Stakeholder]- DBA</re>',
'<re>[DB Stakeholder]- Data Scientist</re>',
'<re>DQM</re>',
'<re>DQM3</re>',
'<re>ISO 11179</re>',
'<re>TTAK.KO-10.0555</re>',
'<re>ISO/IEC 20943-5</re>',
'<re>DQC</re>',
'<re>인덱스</re>',
'<re>다차원 색인구조</re>',
'<re>[다차원 색인]- PAM</re>',
'<re>[다차원 색인]- SAM</re>',
'<re>[다차원 색인]- 인덱스 구조(B,R,T-Tree)</re>',
'<re>[다차원 색인]- Hilbert R-Tree</re>',
'<re>Hashing</re>',
'<re>Clustering Factor</re>',
'<re>NewSQL DBMS</re>',
'<re>데이터 리터러시(Data Literacy)</re>',
'<re>Join</re>',
'<re>Hint</re>',
'<re>데이터 표준화</re>',
'<re>데이터 표준 관리도구</re>',
'<re>DB Tunning</re>',
'<re>[DBMS 최적화]- Optimizer</re>',
'<re>[DBMS 최적화]- Data Block Access</re>',
'<re>[DBMS 최적화]- Parallel Operation</re>',
'<re>Database View</re>',
'<re>ERD</re>',
'<re>관계형 데이터 모델</re>',
'<re>연결함정</re>',
'<re>[빅데이터]- Edge Analytics</re>',
'<re>[빅데이터]- R-HIVE</re>',
'<re>[빅데이터]- NoSQL</re>',
'<re>[NoSQL]- Column Family Store</re>',
'<re>[NoSQL]- Graph Database</re>',
'<re>[빅데이터][NoSQL]- 데이터 모델링 패턴</re>',
'<re>[빅데이터][NoSQL]- CAP 이론</re>',
'<re>[빅데이터][NoSQL]- PACELC</re>',
'<re>[빅데이터][NoSQL]- NoSQL RDBMS 비교</re>',
'<re>[빅데이터][NoSQL]- BASE</re>',
'<re>[NoSQL]- Spatial DB</re>',
'<re>[빅데이터]- 빅데이터 개인정보보호 가이드라인</re>',
'<re>[빅데이터]- Hadoop</re>',
'<re>[빅데이터]- HDFS</re>',
'<re>[빅데이터]- Hadoop EcoSystem</re>',
'<re>[빅데이터]- ISO 20547</re>',
'<re>[빅데이터]- BigData 분석 방법론</re>',
'<re>[빅데이터]- DCAT</re>',
'<re>[빅데이터]- 데이터 플래그십</re>',
'<re>[빅데이터]- 단순 대치법</re>',
'<re>[빅데이터]- 결측값</re>',
...
'<re>[통계]- 통계 관련 데이터</re>',
'<re>[통계]- 통계 Overview</re>',
'<re>[통계]- 데이터 분석 전처리</re>',
'<re>[통계][전처리]- 기술통계</re>',
'<re>[통계][전처리]- 추론통계</re>',
'<re>[통계]- 표본 추출 방법</re>',
'<re>[통계]- 특이정보, Outlier</re>',
'<re>[통계]- 재현정보</re>',
'<re>[통계]- 차등정보보호</re>',
'<re>[통계]- 분석기법</re>',
'<re>[통계][분석기법]- 회귀 분석</re>',
'<re>[통계][회귀 분석]- 다변량 회귀 분석</re>',
'<re>[통계][회귀 분석]- 선형회귀모형 추론 가정</re>',
'<re>[통계][분석기법]- 상관 분석</re>',
'<re>[통계][분석기법]- 주성분 분석</re>',
'<re>[통계][분석기법]- 다차원 척도법</re>',
'<re>[통계][분석기법]- 군집분석</re>',
'<re>[통계][분석기법][군집분석]- 계층적 군집분석</re>',
'<re>[통계][분석기법][군집분석]- 비계층적 군집분석</re>',
'<re>[통계][분석기법]- 판별 분석</re>',
'<re>[통계][분석기법]- 연관규칙</re>',
'<re>[통계][분석기법][연관규칙]- Apriori Algorithm</re>',
'<re>[통계]- 타당도</re>',
'<re>[통계]- 신뢰도</re>',
'<re>[통계]- CLT</re>',
'<re>[통계]- 추정이론(estimation theory)</re>',
'<re>[통계]- 가설검정 (Statistical Hypothesis Testing)</re>',
'<re>[통계]- 베이즈 정리</re>',
'<re>Data Mining</re>',
'<re>[Data Mining]- Text Mining</re>',
'<re>[Data Mining][Text]- 로봇 저널리즘</re>',
'<re>[Data Mining]- 단어구름(Word Cloud)</re>',
'<re>[Data Mining]- Co-Word Analysis</re>',
'<re>[Data Mining]- 데이터 분석 기술</re>',
'<re>[Data Mining]- 토픽 모델링</re>',
'<re>[Data Mining]- Opinion Mining</re>',
'<re>[Data Mining]- System Dynamics</re>',
'<re>[Data Mining]- CRISP-DM</re>',
'<re>데이터 사이언스</re>',
'<re>반정형 데이터</re>',
'<re>RAID</re>',
'<re>DMBOK</re>',
