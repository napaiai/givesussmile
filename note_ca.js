...
'<re>[OS]- Dual Mode</re>',
'<re>Unix OS</re>',
'<re>[Unix OS]- File System</re>',
'<re>[Unix OS]- Permission</re>',
'<re>[Unix OS]- Linux ID</re>',
'<re>[Unix OS]- Super Block</re>',
'<re>[Unix OS]- i-node Block</re>',
'<re>[Unix OS]- I/O Model</re>',
'<re>[Unix OS][Synchronous I/O]- Blocking I/O</re>',
'<re>[Unix OS][Synchronous I/O]- NonBlocking I/O</re>',
'<re>[Unix OS][Synchronous I/O]- I/O Multiplexing</re>',
'<re>[Unix OS]- Asynchronous I/O</re>',
'<re>IO 장치 제어 방식</re>',
'<re>[IO 장치 제어][CPU]- Programmed I/O</re>',
'<re>[IO 장치 제어][CPU]- Interrupt Driven I/O</re>',
'<re>[IO 장치 제어]- DMA</re>',
'<re>Loader</re>',
'<re>Dispatcher</re>',
'<re>[Dispatcher]- 운영체제 문맥, 문맥교환</re>',
'<re>Interrupt</re>',
'<re>[Interrupt]- OS 역할</re>',
'<re>[Interrupt]- Maskable Interrupt</re>',
'<re>[Interrupt]- Non Maskable Interrupt</re>',
'<re>[Interrupt]- Vectored Interrupt</re>',
'<re>Memory 영역</re>',
'<re>Memory Leak</re>',
'<re>OS Scheduler</re>',
'<re>Process</re>',
'<re>PCB</re>',
'<re>Thread</re>',
'<re>Buffering / Spooling</re>',
'<re>결함허용 시스템(FTS:Fault Tolerant System)</re>',
'<re>고가용성(High Availability)</re>',
'<re>우선순위 역전현상</re>',
'<re>WDT</re>',
'<re>반도체 종류</re>',
'<re>메모리 관리 기법</re>',
'<re>[메모리 관리]- 할당 기법</re>',
'<re>[메모리 관리][할당 기법][연속 로딩]- 단일 분할 할당</re>',
'<re>[메모리 관리][할당 기법][연속 로딩]- 다중 분할 할당</re>',
'<re>[메모리 관리][할당 기법]- 분산 로딩 기법(Virtual Memory, Paging, Segmentation)</re>',
'<re>[메모리 관리][할당 기법]- 단편화</re>',
'<re>[메모리 관리]- Page Fault</re>',
'<re>메모리 인터리빙</re>',
'<re>[커널 메모리 할당]- Slab allocator</re>',
'<re>[커널 메모리 할당]- Buddy allocator</re>',
'<re>[Virtual Memory]- Mapping</re>',
'<re>가상 메모리 페이지 교체 알고리즘</re>',
'<re>[가상 메모리 페이지 교체 알고리즘]- 벨라디 변이(Belady\'s Anomaly)</re>',
'<re>[가상 메모리 페이지 교체 알고리즘]- 스레싱(Thrashing)</re>',
'<re>[가상 메모리 페이지 교체 알고리즘][Thrashing]- Working set</re>',
'<re>[가상 메모리 페이지 교체 알고리즘][Thrashing]- PFF</re>',
'<re>Cache Memory</re>',
'<re>[Cache]- Cache Mapping</re>',
'<re>[Cache]- Cache 일관성</re>',
'<re>[Cache]- Directory Protocol</re>',
'<re>[Cache]- Snoopy Protocol</re>',
'<re>[Cache]- MESI</re>',
'<re>PCI Express</re>',
'<re>DRAM</re>',
'<re>SRAM</re>',
...
'<re>FeRAM</re>',
'<re>병렬 컴퓨팅</re>',
'<re>[병렬 컴퓨팅]- Flynn 분류</re>',
'<re>[병렬 컴퓨팅]- 기억장치 액세스 분류</re>',
'<re>[병렬 컴퓨팅]- 상호 연결망</re>',
'<re>[병렬 컴퓨팅][기억장치 액세스]- UMA</re>',
'<re>[병렬 컴퓨팅][기억장치 액세스]- NUMA</re>',
'<re>[병렬 컴퓨팅][기억장치 액세스]- NORMA</re>',
'<re>[병렬 컴퓨팅][기억장치 액세스]- COMA</re>',
'<re>ARM</re>',
'<re>분기 예측 기술</re>',
'<re>[분기 예측]- 분기 방향 예측</re>',
'<re>[분기 예측]- 분기 목적지 예측</re>',
'<re>[분기 예측]- 예측 실행</re>',
'<re>GPGPU</re>',
'<re>[GPGPU]-CUDA</re>',
'<re>Cloud GPU</re>',
'<re>Many core CPU</re>',
'<re>Fabric Computing</re>',
'<re>양자컴퓨팅</re>',
'<re>Tiny OS</re>',
'<re>Nano Qplus</re>',
'<re>Flash Memory</re>',
'<re>NAND Flash Memory</re>',
'<re>eMMC</re>',
'<re>UFS</re>',
'<re>MoS</re>',
'<re>CPU</re>',
'<re>뉴로모픽</re>',
'<re>[CPU]- MajorState</re>',
'<re>CPU Scheduling</re>',
'<re>[CPU Scheduling]- 비선점 스케줄링</re>',
'<re>[CPU Scheduling]- 선점 스케줄링</re>',
'<re>[CPU Scheduling]- Process Aging</re>',
'<re>[RTOS Scheduling]- RM</re>',
'<re>[RTOS Scheduling]- EDF</re>',
'<re>Disk 스케줄링</re>',
'<re>[Disk 스케줄링]- 알고리즘 유형</re>',
'<re>리소스 동기화</re>',
'<re>[리소스 동기화]- 상호배제</re>',
'<re>[리소스 동기화][상호배제]- 세마포어, 모니터</re>',
'<re>[리소스 동기화][상호배제]- Swap Test & Set()</re>',
'<re>[리소스 동기화][상호배제]- 데커 피터슨 램포드</re>',
'<re>[리소스 동기화]- 운영체제의 크리티컬섹션</re>',
'<re>[리소스 동기화][상호배제]- 뮤텍스</re>',
'<re>[리소스 동기화]- 스핀락</re>',
'<re>[리소스 동기화]- 교착상태</re>',
'<re>[교착상태][회피]- 은행가알고리즘</re>',
'<re>[교착상태][회피]- Wait-Die, Wound-Wait</re>',
'<re>[교착상태][발견]- 자원할당 그래프</re>',
'<re>[InMemory]- In-Memory Computing</re>',
'<re>[InMemory]- In-Memory DataGrid</re>',
'<re>[InMemory]- In-Memory Database</re>',
'<re>Auto Scale Up, Out</re>',
'<re>Library</re>',
'<re>[Library]- DLL</re>',
'<re>정규 표현식</re>',
'<re>유한 오토마타</re>',
'<re>엑사스케일 컴퓨팅 시스템</re>',
'<re>Thunderbolt Interface</re>',
);
